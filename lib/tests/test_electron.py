...
def test_electron_integral_ground_level():
    ...
    el = lib.electron.Electron(config, lib.potential.CoulombPotential(config), 1, 0, 0)
...
def test_electron_integral_higher_energy_levels():
    ...
    el = lib.electron.Electron(config, lib.potential.CoulombPotential(config), 2, 0, 0)
    res = np.sum((abs(el.psi) ** 2) * (config.dx) * (config.dy))
    assert floateq(res, 1)

    el = lib.electron.Electron(config, lib.potential.CoulombPotential(config), 2, 1, 0)
    res = np.sum((abs(el.psi) ** 2) * (config.dx) * (config.dy))
    assert floateq(res, 1)

    el = lib.electron.Electron(config, lib.potential.CoulombPotential(config), 3, 0, 0)
    res = np.sum((abs(el.psi) ** 2) * (config.dx) * (config.dy))
    assert floateq(res, 1)

    el = lib.electron.Electron(config, lib.potential.CoulombPotential(config), 3, 2, 1)
    res = np.sum((abs(el.psi) ** 2) * (config.dx) * (config.dy))
    assert floateq(res, 1)
